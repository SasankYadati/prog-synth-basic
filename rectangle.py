...
def even_better_writer(spec):
    inside_coords = [coord for coord,bool in spec if bool]
    if inside_coords == []:
        return random_writer(spec)
    row_coords = [coord[0] for coord in inside_coords]
    col_coords = [coord[1] for coord in inside_coords]
    T, D = min(row_coords), max(row_coords)
    L, R = min(col_coords), max(col_coords)
    return [T, D, L, R]
...
if __name__ == '__main__':
    ...
    synthesizer = get_synthesizer(better_writer, is_correct, 1000)
    res = synthesizer(spec1)
    if res is not None:
        print(res)
